...
- [🛠️ Как устроена очистка ACL](#️-как-устроена-очистка-acl)
  ...
  - [📌 Владелец каталога](#-владелец-каталога)
...
Перед принятием решения о сохранении, удалении или обновлении наследования
каждая запись проходит последовательную классификацию.
...
## 🔀 Как классификация влияет на действие

После определения `PrincipalType` скрипт учитывает признак `IsInherited` и
значение параметра `RefreshStaleInheritance`.

| `PrincipalType` | `IsInherited` | Действие |
|-----------------|---------------|----------|
| `Group` | Любое значение | Правило сохраняется |
| `SystemPrincipal` | Любое значение | Правило сохраняется |
| `Unknown` | Любое значение | Правило сохраняется для безопасной проверки |
| `User` или `OrphanedSid` | `false` | Явная ACE добавляется в `PlannedRules` и планируется к удалению |
| `User` или `OrphanedSid` | `true` | При включённом обновлении ACE добавляется в `PlannedInheritanceRefreshRules` |
| `User` или `OrphanedSid` | `true` | При отключённом обновлении ACE добавляется в `SkippedInheritedRules` |

> [!NOTE]
> В режиме **WhatIf** выполняется только построение плана. Удаление и обновление
> наследования происходят только в режиме **Apply** после разрешения `ShouldProcess`.
...
| `User` | Доменный или локальный пользователь. Явное правило планируется к удалению, унаследованное — к обновлению наследования. |
| `OrphanedSid` | SID не сопоставлен существующей учётной записи. Явное правило планируется к удалению, унаследованное — к обновлению наследования. |
...
> [!NOTE]
> Поля `PrincipalType` и `LookupMethod` используются только для определения типа
> субъекта безопасности. Решение о сохранении, удалении или обновлении наследования
> принимается на основании `PrincipalType`, `IsInherited` и
> `RefreshStaleInheritance`.
...
| `WhatIf` | Построен план удаления и обновления наследования без изменения ACL. |
...
> [!IMPORTANT]
> Поле `Detail` используется только для диагностики. Решение о сохранении,
> удалении или обновлении наследования принимается по значениям `PrincipalType`,
> `IsInherited` и `RefreshStaleInheritance`.
...
## 📊 Как читать результаты сравнения WhatIf и Apply

Скрипт из раздела **«Сравнение результатов WhatIf и Apply»** сначала выводит
четыре итоговых значения:

| Поле | Что увидит администратор |
|------|--------------------------|
| `PlannedRemoval` | Количество явных пользовательских и осиротевших ACE, найденных в журнале WhatIf |
| `Removed` | Количество явных ACE, фактически удалённых в режиме Apply |
| `PlannedRefresh` | Количество унаследованных ACE, для которых WhatIf запланировал возможное обновление наследования |
| `Refreshed` | Количество унаследованных ACE, фактически исчезнувших после принудительного обновления наследования |

После общей статистики выводятся:

1. папки, ACL которых действительно был изменён;
2. полный перечень фактически удалённых явных правил;
3. полный перечень правил, исчезнувших после принудительного обновления наследования.

### Все запланированные явные правила удалены
...
Равные значения означают, что все явные правила, найденные в режиме **WhatIf**,
были успешно удалены в режиме **Apply**.

Если значение `Removed` меньше `PlannedRemoval`, проверьте:

- не было ли отклонено подтверждение `ShouldProcess`;
- отсутствуют ли результаты со статусом `Failed`;
- не изменился ли ACL между запусками WhatIf и Apply;
- совпадали ли `Path`, исключения и остальные параметры обоих запусков.

### PlannedRefresh больше нуля, а Refreshed равен нулю
...
Такой результат сам по себе **не является ошибкой**.

В режиме **WhatIf** явная ACE на родителе фактически не удаляется, поэтому скрипт
видит её унаследованные копии во всех дочерних папках и включает их в
`PlannedRefresh`.

В режиме **Apply** обработка выполняется сверху вниз. После удаления явной ACE
на родителе Windows обычно автоматически удаляет её унаследованные копии.
Когда скрипт доходит до дочерних папок, нежелательных ACE там уже нет, поэтому
принудительное обновление не выполняется и значение `Refreshed` остаётся равным
нулю.

Если в перечне изменённых папок отображаются только каталоги с ненулевым
`RemovedRuleCount`, а `RefreshedInheritedRuleCount` равен нулю, это подтверждает,
что Windows распространила изменения автоматически.

### Refreshed больше нуля
...
Это означает, что для части унаследованных правил автоматического обновления
Windows оказалось недостаточно. Скрипт принудительно обновил наследование, после
чего четыре нежелательные ACE исчезли. Их пути и параметры отображаются в
`RefreshedInheritedRules`.

`PlannedRefresh` и `Refreshed` не обязаны совпадать: первый показатель получен из
предварительного снимка WhatIf до очистки родителей, а второй отражает только
фактически потребовавшиеся принудительные обновления в Apply.

> [!IMPORTANT]
> Для окончательной проверки после завершения Apply повторно запустите WhatIf.
> Ожидаемые значения: `PlannedRemovalCount = 0` и
> `PlannedInheritanceRefreshRuleCount = 0`.
