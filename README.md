...
На файловых серверах, которые эксплуатируются несколько лет, практически всегда появляется одна и та же проблема — большое количество явных пользовательских разрешений.

Со временем сотрудники увольняются, учетные записи удаляются, появляются устаревшие **SID**, а доступ к папкам становится всё сложнее сопровождать.
...
**NTFS-ACL-Group-Cleanup** автоматизирует этот процесс.

Скрипт анализирует каждую запись **ACL**, определяет её тип и удаляет только те разрешения, которые соответствуют заданным правилам, не затрагивая системные объекты безопасности и групповые разрешения.

---

# ❓ Зачем нужен этот проект

Во многих организациях структура доступа строится на комбинации:

- групп безопасности;
- индивидуальных разрешений пользователей;
- наследуемых разрешений;
- временных исключений.

Через несколько лет эксплуатации появляются:

- устаревшие **SID**;
- большое количество явных пользовательских разрешений;
- дублирующиеся записи;
- сложности при аудите безопасности;
- увеличение времени сопровождения файлового сервера.

Этот проект позволяет привести разрешения к единообразному состоянию без полного пересоздания **ACL**.
